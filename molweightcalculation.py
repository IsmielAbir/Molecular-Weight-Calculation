...
def calculate_gui():
    ...
    try:
        result = calculate_weight(formula)
...
root = tk.Tk()
root.title("Molecular Weight Calculator")
root.geometry("600x400")

tk.Label(root, text="Enter Molecular Formula:", font=("Helvetica", 12)).pack(pady=10)
entry = tk.Entry(root, font=("Helvetica", 12), width=40)
entry.pack(pady=5)

tk.Button(root, text="Calculate", command=calculate_gui, font=("Helvetica", 12)).pack(pady=10)

result_text = tk.Text(root, wrap=tk.WORD, font=("Courier", 10), height=12)
